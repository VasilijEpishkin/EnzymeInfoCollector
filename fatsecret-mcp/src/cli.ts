...
class FatSecretOAuthConsole {
  ...
  async setupCredentials(): Promise<void> {
    ...
    if (this.config.clientId && this.config.clientSecret) {
      ...
      console.log(`Client Secret: ${this.config.clientSecret.substring(0, 8)}...`);
  ...
  async runOAuthFlow(): Promise<void> {
    ...
    try {
      ...
      if (!accessResponse.oauth_token || !accessResponse.oauth_token_secret) {
        throw new Error("Invalid response from access token endpoint. Please try again.");
      }
  ...
  async run(): Promise<void> {
    try {
      ...
      console.log("This utility will help you authenticate with the FatSecret API.\n");

      await this.setupCredentials();

      const runOAuth = await this.promptUser("Do you want to authenticate a user now? (y/n): ");
