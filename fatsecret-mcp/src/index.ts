...
class FatSecretMCPServer {
  ...
  private readonly v2BaseUrl = "https://platform.fatsecret.com/rest/2/";
  ...
  private readonly oauth2TokenUrl = "https://oauth.fatsecret.com/connect/token";
  ...
  private async getOAuth2Token(): Promise<string> {
    if (
      ...
      Date.now() < this.config.oauth2TokenExpiry
    ) {
    ...
    const response = await fetch(this.oauth2TokenUrl, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new McpError(
        ...
        `OAuth 2.0 token error: ${response.status} - ${text}`,
      );
    }

    const tokenData = await response.json() as any;

    this.config.oauth2AccessToken = tokenData.access_token;
    // Subtract 60 seconds as a safety margin before expiry
    this.config.oauth2TokenExpiry = Date.now() + (tokenData.expires_in - 60) * 1000;
    await this.saveConfig();

    return tokenData.access_token;
  }

  private async makeV2ApiRequest(
    method: string,
    params: Record<string, string> = {},
  ): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`FatSecret v2 API error: ${response.status} - ${text}`);
    }
  ...
  private setupToolHandlers() {
        ...
        tools: [
          ...
          },
          {
            ...
            inputSchema: {
              type: "object",
              properties: {
                callbackUrl: { type: "string", description: 'OAuth callback URL (use "oob" for out-of-band)', default: "oob" },
              },
            },
          },
          {
          ...
          },
          {
            ...
            inputSchema: {
              type: "object",
              properties: { foodId: { type: "string", description: "The FatSecret food ID" } },
              required: ["foodId"],
            },
          },
          {
          ...
          },
          {
            ...
            inputSchema: {
              type: "object",
              properties: { recipeId: { type: "string", description: "The FatSecret recipe ID" } },
              required: ["recipeId"],
            },
          },
          {
          ...
          },
          {
            ...
            inputSchema: {
              type: "object",
              properties: { date: { type: "string", description: "Date in YYYY-MM-DD format (default: today)" } },
            },
          },
          {
            name: "add_food_entry",
            ...
            inputSchema: {
              ...
              properties: {
                ...
                mealType: { type: "string", description: "Meal type (breakfast, lunch, dinner, snack)", enum: ["breakfast", "lunch", "dinner", "snack"] },
          ...
          },
          {
            ...
            inputSchema: {
              type: "object",
              properties: { date: { type: "string", description: "Date in YYYY-MM-DD format to specify the month (default: current month)" } },
            },
          },
          {
            name: "search_foods_v2",
            description: "Search for foods using the FatSecret v2 API (requires premier scope). Returns richer data including brand info and more filters.",
            inputSchema: {
              type: "object",
              properties: {
                searchExpression: { type: "string", description: "Search term for foods" },
                pageNumber: { type: "number", description: "Page number for results (default: 0)", default: 0 },
                maxResults: { type: "number", description: "Maximum results per page (default: 20, max: 50)", default: 20 },
                language: { type: "string", description: "Language code for results (e.g. 'en', 'de', 'fr')" },
                region: { type: "string", description: "Region/country code to filter results (e.g. 'US', 'GB')" },
              },
              required: ["searchExpression"],
            },
          },
          {
            name: "get_food_v2",
            description: "Get detailed nutritional information for a food item using v2 API, including vitamins and minerals (requires premier scope).",
            inputSchema: {
              type: "object",
              properties: {
                foodId: { type: "string", description: "The FatSecret food ID" },
          ...
          },
          {
            ...
            description: "Find a food item by its barcode (EAN-13 or UPC-A). Returns the FatSecret food ID (requires premier scope).",
            inputSchema: {
              type: "object",
              properties: { barcode: { type: "string", description: "Barcode number (EAN-13 or UPC-A), e.g. '0012000001086'" } },
              required: ["barcode"],
            },
          },
          {
            name: "search_exercises",
            description: "Search the FatSecret exercise database. Returns exercise types with calorie estimates (requires premier scope).",
            inputSchema: {
              type: "object",
              properties: {
                searchExpression: { type: "string", description: "Search term for exercises (e.g. 'running', 'swimming')" },
                language: { type: "string", description: "Language code (e.g. 'en')" },
              },
              required: ["searchExpression"],
            },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (request.params.name) {
        ...
        case "search_foods_v2":
          return await this.handleSearchFoodsV2(request.params.arguments);
        case "get_food_v2":
          return await this.handleGetFoodV2(request.params.arguments);
        case "get_food_by_barcode":
          return await this.handleGetFoodByBarcode(request.params.arguments);
        case "search_exercises":
          return await this.handleSearchExercises(request.params.arguments);
        default:
  ...
  private async handleSetCredentials(args: any) {
    ...
    await this.saveConfig();
    return { content: [{ type: "text", text: "FatSecret API credentials have been set successfully. You can now start the OAuth flow to authenticate users." }] };
  }
  ...
  private async handleStartOAuthFlow(args: any) {
    ...
    try {
      const response = await this.makeOAuthRequest("POST", this.requestTokenUrl, { oauth_callback: callbackUrl });
  ...
  private async handleCompleteOAuthFlow(args: any) {
    ...
    try {
      const response = await this.makeOAuthRequest("GET", this.accessTokenUrl, { oauth_verifier: args.verifier }, args.requestToken, args.requestTokenSecret);
      ...
      await this.saveConfig();
      return { content: [{ type: "text", text: `OAuth flow completed successfully! You are now authenticated with FatSecret.\n\nUser ID: ${this.config.userId}\n\nYou can now use user-specific tools like get_user_profile, get_user_food_entries, and add_food_entry.` }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to complete OAuth flow: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async handleSearchFoods(args: any) {
    ...
    try {
      const params = { method: "foods.search", search_expression: args.searchExpression, page_number: args.pageNumber?.toString() || "0", max_results: args.maxResults?.toString() || "20", format: "json" };
      const response = await this.makeApiRequest("GET", this.baseUrl, params, false);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to search foods: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async handleGetFood(args: any) {
    ...
    try {
      const params = { method: "food.get", food_id: args.foodId, format: "json" };
      const response = await this.makeApiRequest("GET", this.baseUrl, params, false);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to get food: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async handleSearchRecipes(args: any) {
    ...
    try {
      const params = { method: "recipes.search", search_expression: args.searchExpression, page_number: args.pageNumber?.toString() || "0", max_results: args.maxResults?.toString() || "20", format: "json" };
      const response = await this.makeApiRequest("GET", this.baseUrl, params, false);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to search recipes: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async handleGetRecipe(args: any) {
    ...
    try {
      const params = { method: "recipe.get", recipe_id: args.recipeId, format: "json" };
      const response = await this.makeApiRequest("GET", this.baseUrl, params, false);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to get recipe: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async handleGetUserProfile(args: any) {
    ...
    try {
      const params = { method: "profile.get", format: "json" };
      const response = await this.makeApiRequest("GET", this.baseUrl, params, true);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to get user profile: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async handleGetUserFoodEntries(args: any) {
    ...
    try {
      ...
      const params = { method: "food_entries.get", date: date, format: "json" };
      const response = await this.makeApiRequest("GET", this.baseUrl, params, true);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to get food entries: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async handleAddFoodEntry(args: any) {
    ...
    try {
      const date = this.dateToFatSecretFormat(args.date);
      const params = { method: "food_entry.create", food_id: args.foodId, serving_id: args.servingId, quantity: args.quantity.toString(), meal: args.mealType, date: date, format: "json" };
      const response = await this.makeApiRequest("POST", this.baseUrl, params, true);
      return { content: [{ type: "text", text: `Food entry added successfully!\n\n${JSON.stringify(response, null, 2)}` }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to add food entry: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async handleCheckAuthStatus(args: any) {
    ...
    if (hasCredentials && hasAccessToken) { status = "Fully authenticated"; }
    else if (hasCredentials) { status = "Credentials set, authentication needed"; }
    return { content: [{ type: "text", text: `Authentication Status: ${status}\n\nCredentials configured: ${hasCredentials}\nUser authenticated: ${hasAccessToken}\nUser ID: ${this.config.userId || "N/A"}` }] };
  }
  ...
  private async handleGetWeightMonth(args: any) {
    ...
    try {
      const date = this.dateToFatSecretFormat(args.date);
      const params = { method: "weights.get_month", date: date, format: "json" };
      const response = await this.makeApiRequest("GET", this.baseUrl, params, true);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to get weight entries for month: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }

  private async handleSearchFoodsV2(args: any) {
    if (!this.config.clientId || !this.config.clientSecret) {
      throw new McpError(ErrorCode.InvalidRequest, "Please set your FatSecret API credentials first");
    }
    try {
      ...
      const response = await this.makeV2ApiRequest("GET", params);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to search foods (v2): ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }

  private async handleGetFoodV2(args: any) {
    if (!this.config.clientId || !this.config.clientSecret) {
      throw new McpError(ErrorCode.InvalidRequest, "Please set your FatSecret API credentials first");
    }
    try {
      const params: Record<string, string> = { method: "food.get.v4", food_id: args.foodId };
      if (args.language) params.language = args.language;
      const response = await this.makeV2ApiRequest("GET", params);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to get food (v2): ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }

  private async handleGetFoodByBarcode(args: any) {
    if (!this.config.clientId || !this.config.clientSecret) {
      throw new McpError(ErrorCode.InvalidRequest, "Please set your FatSecret API credentials first");
    }
    try {
      const params: Record<string, string> = { method: "food.find_id_for_barcode", barcode: args.barcode };
      const response = await this.makeV2ApiRequest("GET", params);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to get food by barcode: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  private async handleSearchExercises(args: any) {
    ...
    try {
      const params: Record<string, string> = { method: "exercises.get", search_expression: args.searchExpression };
      if (args.language) params.language = args.language;
      const response = await this.makeV2ApiRequest("GET", params);
      return { content: [{ type: "text", text: JSON.stringify(response, null, 2) }] };
    } catch (error) {
      throw new McpError(ErrorCode.InternalError, `Failed to search exercises: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
